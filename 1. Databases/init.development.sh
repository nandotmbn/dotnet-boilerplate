...
grant_privileges() {
  ...
  docker exec $ctname psql -U postgres -p $PORT -c "GRANT ALL PRIVILEGES ON DATABASE $dbname TO devuser;"
}
...
grant_schema_privileges() {
  ...
  docker exec $ctname psql -U postgres -d $dbname -p $PORT -c "GRANT ALL ON SCHEMA public TO devuser;"
}
