...
grant_privileges() {
  ...
  docker exec $ctname psql -U postgres -p $PORT -c "GRANT ALL PRIVILEGES ON DATABASE $dbname TO user;"
}
...
grant_schema_privileges() {
  ...
  docker exec $ctname psql -U postgres -d $dbname -p $PORT -c "GRANT ALL ON SCHEMA public TO user;"
}
...
PORT=5432
setup_database database 1databases-psql-1

setup_database database 1databases-psql1-1

setup_database database 1databases-psql2-1

setup_database database 1databases-psql3-1

setup_database database 1databases-psql4-1